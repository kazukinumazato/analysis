#!/usr/bin/env python3
"""Validate the cycle-averaged thrust model from a Crobat motor-test bag."""
...
def servo_z_axis_in_sensor(
    command_angle_deg: float,
    servo_zero_angle_deg: float,
    base_z_rotation_deg: float,
) -> np.ndarray:
    theta = math.radians(command_angle_deg - servo_zero_angle_deg)
    ...
    return rotation_z.dot(rotation_x).dot(np.array([0.0, 0.0, 1.0]))


def command_angle_at(
    timestamp: float, servo_times: np.ndarray, servo_angles: np.ndarray
) -> float:
    index = int(np.searchsorted(servo_times, timestamp, side="right") - 1)
    if index < 0:
        raise ValueError("no servo command exists before a flapping-cycle boundary")
    return float(servo_angles[index])


def mean_projected_force(
    start: float,
    end: float,
    z_axis: np.ndarray,
    force_times: np.ndarray,
    ...
    if len(interior_times) < min_force_samples:
        return math.nan, len(interior_times)
    ...
    projected_force = integration_forces.dot(z_axis)
    mean_force = float(np.trapz(projected_force, integration_times) / (end - start))
    return mean_force, len(interior_times)
...
def compute_cycle_results(
    ...
    for cycle_index in range(first_cycle, cycle_limit):
        ...
        command_angle = command_angle_at(start, servo_times, servo_angles)
        theta = command_angle - servo_zero_angle
        z_axis = servo_z_axis_in_sensor(
            command_angle, servo_zero_angle, base_z_rotation
        )
        mean_thrust, sample_count = mean_projected_force(
            start,
            end,
            z_axis,
            force_times,
...
def build_summary(cycles: Sequence[CycleResult]) -> List[dict]:
    ...
    for cycle in cycles:
        grouped[(cycle.pwm, cycle.speed)].append(cycle.mean_thrust)

    summaries: List[dict] = []
    pwm_values = sorted({key[0] for key in grouped})
    for pwm in pwm_values:
        fixed = np.asarray(grouped.get((pwm, 0.0), []), dtype=float)
        if not len(fixed):
            raise ValueError("PWM {:.2f} has no fixed-angle interval".format(pwm))
        fixed_mean = float(np.mean(fixed))
        if math.isclose(fixed_mean, 0.0, abs_tol=1.0e-12):
            raise ValueError("fixed-angle mean is zero for PWM {:.2f}".format(pwm))

        speeds = sorted(speed for key_pwm, speed in grouped if key_pwm == pwm)
        for speed in speeds:
            values = np.asarray(grouped[(pwm, speed)], dtype=float)
            mean_thrust = float(np.mean(values))
            absolute_error = 0.0 if speed == 0.0 else abs(mean_thrust - fixed_mean)
            relative_error = 100.0 * absolute_error / abs(fixed_mean)
            summaries.append(
                {
                    "pwm": pwm,
                    "speed_deg_s": speed,
                    ...
                    "thrust_std_N": sample_std(values),
                    "fixed_mean_thrust_N": fixed_mean,
...
def plot_error(summaries: Sequence[dict]) -> None:
    ...
    axis.set_ylabel("Relative error of cycle-averaged thrust [%]")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare fixed- and time-varying-servo cycle-averaged thrust for "
    ...
    parser.add_argument(
        ...
        type=float,
        default=90.0,
        help="command angle corresponding to theta=0 deg",
    )
    parser.add_argument(
        "--base-z-rotation",
        type=float,
...
def main() -> int:
    ...
    forces, force_sampling_frequency = lowpass_forces(
        force_times, forces, args.cutoff_frequency
    )
    print(
    ...
    for row in summaries:
        print(
            ...
            "mean={mean_thrust_N: .5f} N, error={relative_error_percent:6.2f}%, "
            "cycles={cycle_count}".format(**row)
        )
