...
def subtract_interpolated_motor_off_offset(
    force_times: np.ndarray,
    forces: np.ndarray,
    intervals: Sequence[MotorInterval],
    edge_trim: float,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Subtract a time-interpolated force offset measured while the motor is off."""
    if edge_trim < 0.0:
        raise ValueError("--motor-off-offset-trim must be nonnegative")
    if not intervals:
        raise ValueError("motor-off offset correction requires a motor interval")

    gap_bounds = [(float(force_times[0]), intervals[0].start)]
    gap_bounds.extend(
        (previous.end, following.start)
        for previous, following in zip(intervals[:-1], intervals[1:])
    )
    gap_bounds.append((intervals[-1].end, float(force_times[-1])))
    ...
    for gap_start, gap_end in gap_bounds:
        ...
        if gap_duration <= 0.0:
            continue
        trim = min(edge_trim, 0.2 * gap_duration)
        mask = (force_times > gap_start + trim) & (force_times < gap_end - trim)
        if np.count_nonzero(mask) < 3:
            continue
        offset_times.append(float(np.mean(force_times[mask])))
        offsets.append(np.mean(forces[mask], axis=0))

    if not offsets:
        raise ValueError(
            "no usable motor-off force samples were found; use "
            "--no-motor-off-offset-correction to disable correction"
        )

    offset_time_array = np.asarray(offset_times, dtype=float)
    offset_array = np.vstack(offsets)
    interpolated_offset = np.column_stack(
        [
            np.interp(force_times, offset_time_array, offset_array[:, axis])
            for axis in range(3)
        ]
    )
    return forces - interpolated_offset, offset_time_array, offset_array
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=pwm_flapping_periods(
            "0.70:0.07100,0.75:0.05690,0.80:0.04998"
        ),
        metavar="PWM:T_F,...",
        help=(
            "flapping period for each PWM in seconds "
            "(default: 0.70:0.07100,0.75:0.05690,0.80:0.04998)"
        ),
    )
    parser.add_argument(
        "--cutoff-frequency",
    ...
    parser.add_argument(
        "--motor-off-offset-correction",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "subtract a time-interpolated offset measured in motor-off gaps "
            "(default: enabled)"
        ),
    )
    parser.add_argument(
        "--motor-off-offset-trim",
        type=float,
        default=0.15,
        help=(
            "seconds omitted from both ends of each motor-off gap when "
            "estimating its offset (default: 0.15)"
        ),
    )
    parser.add_argument(
        "--speed-sequence",
...
def main() -> int:
    ...
    if (
        ...
        or args.end_trim < 0.0
        or args.motor_off_offset_trim < 0.0
    ):
        raise ValueError(
            "--settle-time, --end-trim, and --motor-off-offset-trim "
            "must be nonnegative"
        )
    ...
    if args.motor_off_offset_correction:
        forces, offset_times, offsets = subtract_interpolated_motor_off_offset(
            force_times,
            forces,
            intervals,
            args.motor_off_offset_trim,
        )
        offset_span = np.ptp(offsets, axis=0) if len(offsets) > 1 else np.zeros(3)
        print(
            "Motor-off offset correction: {} gaps, trim {:.3f} s, "
            "offset span=({:.5f}, {:.5f}, {:.5f}) N".format(
                len(offset_times),
                args.motor_off_offset_trim,
                offset_span[0],
                offset_span[1],
        ...
        print("Motor-off offset correction: disabled")
    label_intervals(
        intervals,
        args.speed_sequence,
        servo_times,
